...
class culinaria_program extends sql{
	...
	public static function daftar_culinaria_program_new( $arr_parameter = array(), $limit = "" ){
		$sql = "select *, month(program_awal) bulan_awal, day(program_awal) tanggal_awal, year(program_awal) tahun_awal,
					month(program_akhir) bulan_akhir, day(program_akhir) tanggal_akhir, year(program_akhir) tahun_akhir, datediff(program_awal, CURRENT_TIMESTAMP) hari_tersisa
					from culinaria_program LEFT JOIN culinaria_program_image cpi ON cpi.program_id=`culinaria_program`.`program_id` ";
	...
	public static function box_program($template, $template_content, $mode = "", $program_id_excluded = "", $show_padding = false){
		...
		if( $mode == "ADMIN" ){
			...
			include __DIR__ . "/../../lang/". $GLOBALS["lang"] ."_culinaria.php";
			$arr_parameter["/*program_tanggal*/"] = array("", " ( program_awal >= CURRENT_TIMESTAMP /*or (program_awal <= CURRENT_TIMESTAMP and program_akhir >= CURRENT_TIMESTAMP)*/ )");
			$arr_parameter["program_aktif"] = array("=", 1);
			$rs_program = self::daftar_culinaria_program_new( $arr_parameter, 8 );
			while( $program = mysqli_fetch_array( $rs_program, MYSQLI_ASSOC ) ){
				$program['hari_tersisa_label'] = str_replace("#hari#", $program['hari_tersisa'], $lang_hari_program_tersisa);
				$arr_data[] = $program;

			}
			$simg = $arr_data;
			
// 			if(mysqli_num_rows($rs_program) <= 0){
// 			    $simg[] = array(
// 			            "image_url" => "256.jpg",
// 			            "program_awal" => "2021-02-10",
// 			            "program_akhir" => null,
// 			            "program_judul_id" => "",
// 			            "hari_tersisa_label" => "",
// 			            "program_chef" => "default",
// 			            "program_harga" => 0,
// 			        );
// 			}
	...
	private static function render_box_program( $counter, $program, $template, $template_content, $show_padding, $counter_show_default = 4 ){
		...
		$sisa_kursi = mysqli_fetch_array( $rs_sisa_kursi );
