...
function member_data_information($memberemail, $information = "", $hp = "", $verifikasi=1){
	//$sql = "select ". ($information != "" ? $information : "*") ." from membersdata where email = '". main::formatting_query_string( $memberemail ) ."' ";
	$sql = "select ". ($information != "" ? $information : "*") ." from membersdata where (email = '". main::formatting_query_string( $memberemail ) ."' OR HandPhone = '". main::formatting_query_string( $hp ) ."') and verifikasi='". $verifikasi."' ";
	$rs = mysql_query( $sql );
	return $rs;
}
...
function kirim_email($target=SUPPORT_EMAIL,$subject="",$content="", $lampiran=array()){
	...
	$to["Bcc"]='dedi.supatman@modena.com';
...
if( mysql_num_rows( member_data_information( $_POST["e"],"", $_POST["hp"],1 ) ) > 0 ) 
    ...
    $encrypted = bin2hex(openssl_encrypt($pin,'AES-128-CBC', $key));
    $sql = "insert into forgot_pass (email,pin,tipe) values ( '". $_POST["e"] ."' ,'". $pin ."','reg') ";
         mysql_query( $sql );
    	
	$rs = member_data_information( $_POST["e"],"", $_POST["hp"],0 ) ;
	...
	if( mysql_num_rows( $rs ) <= 0 ){
	    ...
	    while( $data = mysql_fetch_array( $rs ) ){
    		$arr_data[] = array("status"=>"OK", "pesan"=>"Tautan Verifikasi sudah berhasil dikirimkan ke alamat email Anda.<br />Silahkan akses inbox email Anda untuk mengaktifkan akun anda.","konsumen_id"=>$data["MemberID"], "email"=>$data["Email"], "nama" => $data["Name"] );
    		$msgc=file_get_contents(__DIR__ . "/../lang/".$lang."_verifikasi_akun.html");
    		$ar=array(
    			"#email#"=>$data["Email"],
    			"#name#" => $data["Name"],
    			"#link#" => "https://www.modena.co.id/user-verification.php?e=". $_POST["e"] ."&pn=". $encrypted
    			);
    		$content = str_replace( array_keys($ar), array_values($ar), $msgc );
    		kirim_email($data["Email"],"Thank you for registering to MODENA, please verify your account now",$content);
    		break;
        }
	}else{
	    while( $data = mysql_fetch_array( $rs ) ){
