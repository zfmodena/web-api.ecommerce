...
if( !isset($_REQUEST["grup"]) || ( isset($_REQUEST["grup"]) && $_REQUEST["grup"] == "" ) )
    ...
    $sql = "select a.id id_produk, a.title nama_awal, a.second_title nama_akhir, concat(a.title, ' - ', second_title) nama_lengkap, a.price harga, replace(replace(replace(replace(a.features, '</div><div>', ';'),'<div>',''),'</div>',''),'<br>','') fitur_id, 
        ...
        where c.mediable_type like '%Product' and c.content_type is null and a.category_id = '". main::formatting_query_string($_REQUEST["id"]) ."' and a.published = 1 and a.title = '". main::formatting_query_string($_REQUEST["grup"]) ."'
        ";
