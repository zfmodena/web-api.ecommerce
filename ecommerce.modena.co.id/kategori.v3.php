...
function category_link($parent_category, $id_kategori){
    ...
    $url_category = product_link( $url_category_nama[0] == "home-appliance" ? ( $url_category_nama[1] != "" ? $url_category_nama[1] : $data["nama_kategori"] ) : $url_category_nama[0] );
...
while($data = mysql_fetch_array($rs, MYSQL_ASSOC)){
    $url_category = category_link($parent_category, $data["id_kategori"]);
...
if( mysql_num_rows($rs) > 0 ){
    ...
    $json["kategori"][] = array("id_kategori" => $arr_id_kategori[ count($arr_id_kategori)-1 ], "nama_kategori" => $arr_nama_kategori[ count($arr_nama_kategori)-1 ]) + ($show_url_category ? array("link" => category_link($parent_category, $_REQUEST["id"])) : array(  ));
