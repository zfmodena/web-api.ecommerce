...
class culinaria_cart extends culinaria_program{
	...
	static function culinaria_nomor_order_baru($email = ""){
	    if( $email != "" ){
	        // dapatkan nomor lama yg belum lanjut pembayaran
	        $rs_nomor_order_lama = mysql_query("select order_no from culinaria_ordercustomer where order_status = 0 and memberid = (select memberid from membersdata where email = '". main::formatting_query_string($email) ."')");
	        if( mysql_num_rows($rs_nomor_order_lama) > 0 ) {
	            $nomor_order_lama = mysql_fetch_array($rs_nomor_order_lama);
	            return $nomor_order_lama["order_no"];
	        }
	    }
		...
		if(mysql_num_rows($check_order_number_oc)>0 || mysql_num_rows($check_order_number_doku)>0 || mysql_num_rows($check_order_number_oc_log)>0 || 
			...
			self::culinaria_nomor_order_baru($email);
		else{
		    if( $email != "" ) 
		        mysql_query("insert into culinaria_ordercustomer(order_no, memberid, order_status, order_date) 
		            select '". main::formatting_query_string($rs_order_number_["order_number"]) ."', memberid, 0, CURRENT_TIMESTAMP from membersdata where email = '". main::formatting_query_string( $email ) ."' ") or die();
	...
	static function browse_culinaria_cart_item( $arr_parameter ){
		$sql = "select *, a.product_price program_harga, (a.product_price*a.quantity) total_amount 
			from culinaria_orderproduct a, culinaria_ordercustomer b/*, culinaria_program c*/ where a.order_id = b.order_id /*and a.product_id = c.program_id*/ ";
	...
	static function daftar_cart_program( $template_data_order, $read_only = false, $billingid = "" ){
		...
		if( count(@$arr_program_id) <= 0 )
            return array("arr_data_order"=>$arr_data_order, "string_data_order" => "", "subtotal" => 0 );
	...
	static function hitung_nilai_diskon_kupon( $arr_data_order, $coupon_code, $mode = "" ){
		...
		if($persen_diskon==0){
			...
			$s_note_coupon_code_noformat = $add_error;
		}
		...
		return array
			(
				...
				"s_note_coupon_code_noformat" => $s_note_coupon_code_noformat,
			);
