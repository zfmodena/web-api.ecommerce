...
/* ############################# metode umum ########################################*/
	...
	*/
	...
	public static function shipment_state($state_id){		
		...
		$str_sql = "";
		if( self::cek_item_stiker() )
			$str_sql = " and state_id in (select state_id from shipment_exception where region_id in (". implode(",", $GLOBALS["arr_sticker_lokasi_valid"]) ."))";
		$sql="select state_id, /*concat('Prov. ', state)*/ state, sortorder, cost, enabled from shipment_state where enabled='1' $str_sql ";
	...
	public static function shipment_exception($state_id, $region){
		$str_sql = "";
		if( self::cek_item_stiker() )
			$str_sql = " and region_id in (". implode(",", $GLOBALS["arr_sticker_lokasi_valid"]) .") ";
		$sql="select b.state_id, b.region_id, b.region from shipment_exception b where b.enabled='1' $str_sql ";
	...
	static function cek_item_stiker( $arr_produk_yang_akan_dicek = array() ){
	    if( count( $arr_produk_yang_akan_dicek ) <= 0 ) $arr_produk_yang_akan_dicek = @$_SESSION["shopping_cart"];
		$return = false;
		
		if( count($arr_produk_yang_akan_dicek) > 0 ){
			
			foreach( $GLOBALS["product_custom"] as $productid => $arr_productid ){
				if( array_key_exists($arr_productid["stickerid"], $arr_produk_yang_akan_dicek ) ){
					$return = true;
					break;
				}
			}
...
*/		
	private static function discount_coupon($code, $enabled, $not_expired){/* return nilai diskon */		
		$return_discount=0;
		$mekanisme_khusus = false;
		$status_double_promo = 0;
		$sql="select coupon_code, discount, /*case unlimited when true then 1 else 0 end*/ unlimited, min_qty, min_sales, coupon_mekanisme, status_double_promo from discount_coupon where  left(coupon_code, 3) <> 'CUL' and coupon_code='".main::formatting_query_string($code)."' and enabled='".$enabled."' ";
		...
		if($return_discount!=0 && !$mekanisme_khusus){
			...
			foreach($_SESSION["shopping_cart"] as $productid=>$qty) {//if(order_online_promo::check_ipc_per_item($productid)) return -1;	
				...
				if(mysql_num_rows($rs)>0){	
					...
					if(mysql_num_rows(order_online_promo::discount_product_source($productid))>0) {
	...
	public static function counting_discount($code, $subtotal){
		$discount_coupon=main::discount_coupon($code, "1", true);
