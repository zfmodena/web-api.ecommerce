...
/* ############################# metode umum ########################################*/
	...
	public static function get_product($memberemail, $order_no, $mode="ONLINE" /* ONLINE || SHOWROOM */){
		...
		if($order_no!="")$sql=str_replace("#order_no#", "and order_id=(select order_id from ordercustomer where order_no='".main::formatting_query_string($order_no)."' #order_source#) ");
...
/*			if(shoppingcart::is_product_exists_inventory($product_id, $order_number))
	...
	public static function set_availability_dan_shipping_info($productid, $propinsi, $kota, $kuantitas, $total_harga){
