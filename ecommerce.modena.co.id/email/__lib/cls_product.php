...
/* ############################# metode umum ########################################*/
	...
	public function product_list($categoryid, $table_target_of_product=""){
		...
		if(isset($this->data["categoryid"]))$sql.=" and a.categoryid = ".$this->data["categoryid"];
		if(isset($this->data["parent_categoryid"]))$sql.=" and d.parentcategoryid = ".$this->data["parent_categoryid"];
	...
	public function product_list_fair($categoryid, $table_target_of_product=""){
		//$sql=SET SESSION sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
		//$sql="SET sql_mode=(SELECT REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', ''))";
		/* paramater : distinct, lang, spec_id, productid, penable, orderby, product_price */
		if(isset($this->data["distinct"]))$sql=" select distinct d.sortorder sortorder_category, a.productid, a.sortorder, a.sortorder_group, a.categoryid, a.name, a.description, a.isnewproduct, a.price, a.penable, a.weight, d.parentcategoryid, e.categoryid parent_parentcategoryid, d.name parentcategoryname, e.name parent_parentcategoryname";
		...
		$sql.=" from product".$table_target_of_product." a left outer join misc_spec_product b on a.productid=b.product_id 
			...
			inner join category d on a.categoryid=d.categoryid
			left outer join category e on d.parentcategoryid=e.categoryid
			where d.parentcategoryid in (".$categoryid.") ";
		if(isset($this->data["spec_id"])){
			foreach($this->data["spec_id"] as $key=>$value){
				$arr_value=preg_split("/,/", $value);
				$sql.=" and (";
		...
		if(isset($this->data["categoryid"]))$sql.=" and a.categoryid = ".$this->data["categoryid"];
		if(isset($this->data["parent_categoryid"]))$sql.=" and d.parentcategoryid in(".$this->data["parent_categoryid"] .")";
	...
	public function product_simulasi($product_custom){
		$sql="SELECT * FROM product WHERE productid in (". implode(",", array_keys($product_custom)) .") ORDER BY categoryid,NAME DESC";
		// echo $sql; exit();
	...
	public function product_spec($productid){
		...
		$result=mysql_query($sql) or die();//echo $sql;//("product_spec query error.<br />".mysql_error());
		return $result;
	}
