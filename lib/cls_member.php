...
/* ############################# metode umum ########################################*/
	public function member_data($memberid){
		...
		$sql="select a.memberid, a.name, a.email, a.password, replace(a.address,'\r\n','') address, /*a.homecity,*/ a.homeregion, c.region homecity, a.homestate, b.state, a.homepostcode, a.homecountry, a.phone, a.handphone, a.milis, date_format(a.dateofbirth, '%d/%m/%Y') dateofbirth_original, date_format(a.dateofbirth, '%d %M %Y') dateofbirth, case a.download_auth when true then 1 else 0 end download_auth, a.membercode
	...
	public static function registered_product($memberemail, $membersproductid, $order_no="", $mode=""){
		$sql="select membersproductid, product, serialnumber, purchaseat, date_format(cast(purchasedate as date), '%d/%m/%Y') purchasedate_original, date_format(cast(purchasedate as date), '%d %M %Y') purchasedate, purchasedate purchasedate_unformatted 
		...
		$sql .= " order by purchasedate_unformatted desc";
	...
	public function member_email($target /*customer | * */){
		...
		include __DIR__ . "/../" . $this->lang;		
		
		$message=file_get_contents(__DIR__ . "/../" . $this->email_template);	
	...
	public function check_login_session($drop_all_session = false /* true : hanya di saat login berhasil */, $session = ""){
		if($drop_all_session){
			$sql = "delete from user_session where email = '". main::formatting_query_string($_SESSION["email"]) ."'";
			mysql_query($sql) or die();
			return $this->__insert_login_session();
		}
		$sql = "select 1 from user_session where session = '". main::formatting_query_string($session)."' and email = '". main::formatting_query_string($_SESSION["email"]) ."'";
		$rs = mysql_query($sql) or die();
		if(mysql_num_rows($rs) > 0) return true;
		else return false;
	}
	
	private function __insert_login_session(){
		startfunction:
		$session = rand(0, 1000000);
		if( $this->check_login_session(false, $session) ) goto startfunction;
		$sql = "insert into user_session(session, email) values(#session#, #email#)";
		$arr_sql = array(
						"#session#" => $session, 
						"#email#"=> "'". main::formatting_query_string($_SESSION["email"]) ."'"
						);
		$sql = str_replace(array_keys($arr_sql), array_values($arr_sql), $sql);
		mysql_query($sql) or die();
		return $session;
	}
