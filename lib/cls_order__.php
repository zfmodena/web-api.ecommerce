...
/* ############################# metode umum ########################################*/
	...
	public static function order_product($order_number, $customer_email){
		$sql="select c.name, a.product_id, a.quantity, a.product_price, a.product_promo, a.discount, a.product_pricepromo, (a.product_pricepromo*a.quantity) total_amount, a.tradein, a.promo_per_item_id, a.note
			from orderproduct a, ordercustomer b, product c where a.order_id=b.order_id and a.product_id=c.productid and b.order_no='".main::formatting_query_string($order_number)."' and
	...
	private static function get_email_address($state_id, $region_id){ // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<----------------------------------- tambahkan email_finance
		$sql="select email_admin, email_wh, email_finance from branch a, branch_service b, shipment_exception c, shipment_state d 
...
//		include $this->discount_object;
		...
		while($rs_product_list_=mysql_fetch_array($rs_product_list)){
			...
			$s_order_content.="<tr>
					<td align=\"center\" id=\"column_product_list\"><img src=\"http://www.modena.co.id/images/".(file_exists(__DIR__."/../images/product/tn".$rs_product_list_["name"].".png")?"product/tn".$rs_product_list_["name"].".png":"product.png")."\" border=\"0\" width=\"100px\" /></td>
					<td valign=\"top\" id=\"column_product_list\"><strong>".$rs_product_list_["name"]."<br />".($rs_product_list_["tradein"]=="1"?"Trade-in *":"").
					($free_gift["remark_".$lang[0]]!=""?"<br /><span style=\"text-decoration:underline\">".$free_gift["remark_".$lang[0]]:"")."</span>
					".($rs_product_list_["note"]!=""?"<br /><span style=\"text-decoration:underline\">".$rs_product_list_["note"]."</span>":"")."</strong></td>
		...
		*/
		...
		if($s_discount_2!="")$s_note_discount_2="<sup>[".(@$s_discount_1!=""?"2":"1")."]</sup> ".$lang_promo_code_discount.", ".
		...
		$order_content=str_replace("#shipping_info#", "<strong>".$rs_order_list_["receiver_name_for_shipping"]." <br />
			...
			".$lang_note.": ".$rs_order_list_["shipping_note"]."</span>", $order_content);
		...
		$s_payment_info=$lang_payment_date.": ".$payment_info["finishtime"]."<br />".
			$lang_payment_status.": <strong>".($payment_info["trxstatus"]!="Success"?$lang_payment_status_notok:$lang_payment_status_ok)."</strong><br />".
