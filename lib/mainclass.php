	...
	if( !$check_session->check_login_session(false, $_SESSION["login_session"]) ) 
		@$maic .= "
			function load_box_login_session_expired(){
				TINY.box.show({iframe:'session_expired.php',boxid:'frameless',width:431,height:219,fixed:true,maskid:'greymask',maskopacity:40,close:false});
			}
			try{
...
// PRODUCT CATALOG	- SHOPPING CART
		...
		// cek apakah stiker atau produk dengan stiker
		if( @$_SESSION["order_no"] != "" ){
			foreach($GLOBALS["product_custom"] as $productid => $arr_product){
				if( $_REQUEST["productid"] == $arr_product["stickerid"] || $_REQUEST["productid"] == $productid  ){
					include_once __DIR__ . "/../sticker-hapus.php";
					hapus_sticker_kadaluwarsa($_SESSION["order_no"], "product_id", $productid);
					// hapus dari produk, so stikernya juga harus dihapus juga
					if( $_REQUEST["productid"] == $productid ){
						unset($_SESSION["shopping_cart"][$arr_product["stickerid"]]);
						$sql = "delete from orderproduct where order_id = (select order_id from ordercustomer where order_no = '". main::formatting_query_string($_SESSION["order_no"]) ."') 
							and product_id = '". main::formatting_query_string($arr_product["stickerid"]) ."'";
						mysql_query($sql);
					}
					break;
				}
...
// MEMBER LOGIN
		...
		require_once "securimage.php";
      	$securimage = new Securimage();      
		if(!$securimage->check($_REQUEST["t_captcha"]) /*true !== true*/) {
			...
			if(mysql_num_rows($rs_member_login)>0){
				...
				//while($rs_shoppingcart_=mysql_fetch_array($rs_shoppingcart)){
				foreach($_SESSION["shopping_cart"] as $rs_shoppingcart_["product_id"] => $rs_shoppingcart_["quantity"]){
				...
				if( @$_REQUEST['v'] != "" || @$_REQUEST['order_number'] != "" ){
					$return_member_login="<script>parent.location.href='". @$_REQUEST["path"] ."?". (@$_REQUEST['v'] != "" ? "v=" . @$_REQUEST['v'] : "") ."';</script>";
				}else
					$return_member_login="<script>__submit('member_login.php', 'cs=". $member_login_cs ."+path=".@$_REQUEST["path"]."+t_discount_coupon=".@$_REQUEST["t_discount_coupon"]."+cb_toc=".@$_REQUEST["cb_toc"]."+order_number=".@$_REQUEST['order_number']."+v=".@$_REQUEST['v']."');</script>";
...
//SEARCH	
		...
		// $sql="BEGIN;";
		$state = explode("|",main::formatting_query_string($_REQUEST['t_state']));
		$sql ="INSERT INTO tradein_konsumen(tradein_regid,tradein_tanggal,tradein_nama,tradein_alamat,tradein_region_id,tradein_state_id,tradein_kodepos,tradein_hp,
			...
			VALUES ('".main::formatting_query_string($dakon['id'])."', '".date("Y-m-d H:i:s")."'
			,'".main::formatting_query_string($_REQUEST['t_name'])."'
			,'".main::formatting_query_string($_REQUEST['t_address'])."'
			,'".main::formatting_query_string($_REQUEST['s_city'])."'
			,'".$state[0]."','".main::formatting_query_string($_REQUEST['t_postalcode'])."'
			,'".main::formatting_query_string($_REQUEST['t_telephone'])."'
			,'".main::formatting_query_string($_REQUEST['t_email'])."'
			,'".main::formatting_query_string($_REQUEST['t_brand'])."'
			,NULL,0,NULL);";
		mysql_query($sql);
		...
		for($i=1;$i<$_REQUEST['produk'];$i++){			
			if(@$_FILES['t_foto_'.$i]["error"] == 0) {
				...
				$cfile = new CURLFile($_FILES['t_foto_'.$i]["tmp_name"],$_FILES['t_foto_'.$i]["type"],$_FILES['t_foto_'.$i]["name"]);
				$action = 'upload';
				$rand = rand(100,99999);
				$id_file = $dakon['id'].'_'.$i;
				$folder = 'tradein';
				$key = sha1('tradein'.$id_file.$rand.'ptim328');
				$post = array(
						'action' => $action,
						'key' => $key,
						'random' => $rand,
				...
				//print_r($post);
				$url = "https://www.modena.co.id/dev/api_upload.php";
				$ch = curl_init();
				...
				$nama_file_lampiran = $server_output['link_public'];
				
				curl_close($ch);
				
				//include '../dev/api_upload_in.php';
				//$nama_file_lampiran = $link_public;
				
				//echo $nama_file_lampiran;
				//exit;
			}else{
				//echo "<script>alert('File Size Limit');</script>";
				mysql_query("delete from tradein_konsumen where tradein_regid='".$dakon['id']."'");
			...
			VALUES ('".$dakon['id']."','".main::formatting_query_string($_REQUEST['t_cat_'.$i])."'
			,'".main::formatting_query_string($_REQUEST['t_jumlah_'.$i])."','".$bongkar."','".$instal."',
			'".$nama_file_lampiran."','".main::formatting_query_string($_REQUEST['t_umur_'.$i])."');";
		...
		$sql ="select a.tradein_to_categoryid, b.parentcategoryid, b.Name from tradein_matrix a
				...
				where a.tradein_from_categoryid='".main::formatting_query_string($_REQUEST['catid'])."' order by b.parentcategoryid ";
		...
		$sql="SELECT tradein_regid,tradein_tanggal,tradein_nama,tradein_alamat,tradein_region_id,tradein_state_id,tradein_kodepos,tradein_hp,
			...
			where tradein_regid='".main::formatting_query_string($_REQUEST['id'])."'";
		...
		$sql="select coupon_id,coupon_code,enabled,expired_date,discount,unlimited,remark_id,remark_en,
					min_qty,min_sales,status_double_promo,image,coupon_mekanisme,qty_quota from discount_coupon where coupon_code = '".main::formatting_query_string($kon['tradein_coupon_code'])."'";
		$rs_kon = mysql_query($sql) or die();
		$kon = mysql_fetch_array($rs_kon);
		...
		$sql = "SELECT tradein_regid,tradein_categoryid,tradein_kuantitas,tradein_bongkar,tradein_pasang,tradein_foto,tradein_usiaproduk,tradein_alasanproduk
			from tradein_produk where tradein_regid = '".main::formatting_query_string($_REQUEST['id'])."'";
		...
		$_REQUEST['t_brand'] = $kon['tradein_brand'];
		$_REQUEST['t_pocer'] = $kon['tradein_coupon_code'];
		$diskon=$kon['tradein_brand']=='1'?'0.35':'0.3';
		$_REQUEST['lang'] = $lang;
		...
		if($lum>0){
			...
			$sql ="UPDATE tradein_produk SET tradein_alasanproduk = '".main::formatting_query_string($alas)."'
					where tradein_regid = '".main::formatting_query_string($_REQUEST['id'])."'
					and tradein_categoryid = '".main::formatting_query_string($_REQUEST['cat'])."' ;";
			mysql_query($sql);
			if($lum==1) goto per;
		...
		per:
		$sql = "SELECT tradein_regid,tradein_categoryid,tradein_kuantitas,tradein_bongkar,tradein_pasang,tradein_foto,tradein_usiaproduk,tradein_alasanproduk
			from tradein_produk where tradein_regid = '".main::formatting_query_string($_REQUEST['id'])."' and tradein_alasanproduk = 0";
		...
		if($jum>=1){
			...
			$copid = $rs1['id'];
			$sql = "select CONVERT(SUBSTR(coupon_code,7,4),int) jum from discount_coupon where coupon_code like '".main::formatting_query_string($cek)."%'";
			$rs1 = mysql_query($sql);
			$arr_cek = array();
			...
			$sql ="INSERT INTO discount_coupon (coupon_id,coupon_code,enabled,expired_date,discount,unlimited,remark_id,remark_en,
					...
					VALUES ('".main::formatting_query_string($copid)."','".main::formatting_query_string($kupon)."','1','".main::formatting_query_string($tgl)." 23:59:59',NULL,'0','TRADE-IN','TRADE-IN','1','0','0','',
					'tradein_modena_nonmodena|".main::formatting_query_string($diskon)."',NULL);";
			mysql_query($sql);
			
			$sql ="UPDATE tradein_konsumen SET tradein_coupon_code= '".main::formatting_query_string($kupon)."',
			...
			$sql ="UPDATE tradein_konsumen SET tradein_coupon_code= NULL,
						tradein_verifikasi= '1',
						tradein_verifikasi_tanggal= '".date("Y-m-d H:i:s")."',
						tradein_alasan= '".main::formatting_query_string($_REQUEST['alasan'])."'
					where tradein_regid = '".main::formatting_query_string($_REQUEST['id'])."';";
...
elseif( $page == "appliances.php" ){
    $page_title = "Penuhi kebutuhan rumah tangga dengan peralatan berkualitas | MODENA";
    $arr_page_mdescription[ $page ] = "Cari dan beli produk terbaik untuk Anda dari alat memasak, kebersihan, dan pendingin. Mulai cari di sini untuk kompor, oven, kulkas, vacuum cleaner, mesin cuci";

}elseif( $page == "professional.php" ){
    $page_title = "Produk untuk professional restoran hotel toko | MODENA";
    $arr_page_mdescription[ $page ] = "Peralatan hotel, restoran, cafe & bakery (horecaba) dan usaha kecil & menengah (UKM/UMKM) dengan kualitas teratas. Cek ketersediaan dan harganya di sini.";
    ...
    //$sql = "select c.`name` nama_kategori, c.`description` deskripsi, c.judul from category a left outer join (select min(sortorder) sortorder, parentcategoryid from category where `ENABLEd` = 1 group by parentcategoryid) b on a.categoryid = b.parentcategoryid left outer join category c on b.parentcategoryid = c.parentcategoryid and b.sortorder = c.sortorder and c.enabled = 1 where a.categoryid = '". main::formatting_query_string(@$_REQUEST["categoryid"]) ."'";
    $sql = "select a.`name` nama_kategori, a.`description` deskripsi, a.judul from category a where a.categoryid = '". main::formatting_query_string(@$_REQUEST["categoryid"]) ."'";
    if( @$_REQUEST["sub_categoryid"] != "" )
	    $sql = "select a.`Name` nama_kategori, b.`Name` nama_subkategori, a.categoryid, b.description deskripsi, b.judul from category a inner join category b on a.CategoryID = b.ParentCategoryID where b.CategoryID = '". main::formatting_query_string($_REQUEST["sub_categoryid"]) ."'";
	...
	if( $data_title["judul"] != "" ) $page_title = $data_title["judul"];
	...
	$data_title = mysql_fetch_array( mysql_query($sql) );
	$page_title = "MODENA Indonesia - " . ucwords(strtolower($data_title["nama_subkategori"])) /*. ( in_array($data_title["categoryid"], array(1,2,3)) ? " Appliances" : " Professional" )*/ . " > " . $data_title["nama_product"];
	//if( $data_title["judul"] != "" ) $page_title = $data_title["judul"];
    ...
    $page_title = "Daftar promo dan potongan harga barang rumah tangga | MODENA";
    $arr_page_mdescription[ $page ] = "Simak daftar promo potongan harga produk-produk MODENA di-update berkala mulai dari mesin cuci, pengering pakaian, alat cuci piring, vacuum cleaner, kulkas, kompor";
    if( @$_REQUEST["categoryid"] == 135 ){
        $page_title = "Daftar promo dan potongan harga produk profesional | MODENA";
        $arr_page_mdescription[ $page ] = "Daftar promo potongan harga produk profesional MODENA di-update berkala mesin pendingin, kompor, chiller, untuk kebutuhan restoran, hotel, kedai kopi, kafe";
    }
    
}else
    $page_title = "MODENA Indonesia Official Site";
